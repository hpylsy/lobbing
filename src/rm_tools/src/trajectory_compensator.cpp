...
bool TrajectoryCompensator::compensate(
  const Eigen::Vector3d & target_position, double & pitch) const noexcept
{
  ...
  for (int i = 0; i < iteration_times; ++i) {
    ...
    dh = target_height - impact_height;  //下坠高度
    if (std::abs(dh) < 0.01) {
      break;
    }
    iterative_height += dh;
...
double ResistanceCompensator::calculateTrajectory(const double x, const double angle) const noexcept
{
