...
namespace rm_tools
{
bool ManualCompensator::updateMap(
  ...
  const double yaw_offset)
{
  ...
  if (target_dist_node == angle_offset_map_.end()) {
    ...
    auto target_height_node = std::find_if(
      target_dist_node->height_map.begin(), target_dist_node->height_map.end(),
...
std::vector<double> ManualCompensator::angleHardCorrect(const double dist, const double height)
{
  auto target_dist_node = std::find_if(
    angle_offset_map_.begin(), angle_offset_map_.end(),
    [&](const DistMapNode & dist_node) { return dist_node.dist_region.checkPoint(dist); });
