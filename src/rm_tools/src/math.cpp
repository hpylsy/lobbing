...
void rotation_matrix(geometry_msgs::msg::Quaternion Quaternion, cv::Mat & dst, bool inverse)
{
  geometry_msgs::msg::Quaternion & q = Quaternion; // 别名
  return rotation_matrix(Eigen::Quaterniond(q.w, q.x, q.y, q.z), dst, inverse);
}

void rotation_matrix(Eigen::Quaterniond q, cv::Mat & dst, bool inverse)
{
  q.normalize();
  Eigen::Matrix3d R_eigen = q.toRotationMatrix();

  cv::eigen2cv(R_eigen, dst);
...
geometry_msgs::msg::Quaternion ros_quaterniond(const cv::Mat & rvec)
{
  Eigen::Quaterniond q = quaterniond(rvec);  // 用Eigen处理
...
Eigen::Quaterniond quaterniond(const cv::Mat & R)
{
  ...
  if(R.rows == 3 && R.cols == 1) {
    ...
    RCLCPP_ERROR(rclcpp::get_logger("rm_tools"),"用于转换成四元数的cv::Mat格式不对！既不是旋转向量也不是旋转矩阵");
    return Eigen::Quaterniond(1.0, 0.0, 0.0, 0.0); // 单位四元数，表示不发生旋转
  }

  Eigen::Matrix3d rmat_eigen;
  cv::cv2eigen(rmat, rmat_eigen);

  Eigen::Quaterniond q(rmat_eigen);
  return q;
}

Eigen::Vector3d eulers(cv::Mat R, int axis0, int axis1, int axis2, bool extrinsic){
  Eigen::Quaterniond q = quaterniond(R);
  Eigen::Vector3d eu = rm_tools::eulers(q, axis0, axis1, axis2, extrinsic);
  return eu;
}
