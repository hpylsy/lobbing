  ...
  serial_driver_{new drivers::serial_driver::SerialDriver(*owned_ctx_)}
{
  ...
  // Create Subscription
  // aim_sub_ = this->create_subscription<auto_aim_interfaces::msg::GimbalCmd>(
  //   "armor_planner/cmd_gimbal", rclcpp::SensorDataQoS(),
  //   std::bind(&RMSerialDriver::sendArmorData, this, std::placeholders::_1));
  aim_sub_.subscribe(this, "/armor_planner/cmd_gimbal", rclcpp::SensorDataQoS().get_rmw_qos_profile());
  aim_time_info_sub_.subscribe(this, "/time_info/aim");

  aim_sync_ = std::make_unique<AimSync>(aim_syncpolicy(5000), aim_sub_, aim_time_info_sub_);
    ...
    try {
      ...
      if (header[0] == 0x5A) {
        ...
        if (crc_ok) {
          ...
          // RCLCPP_INFO(
          //   get_logger(), "Bullet speed: %f, Task mode: %d, Theory task: %s", packet.bullet_speed,
